...
var lerb = (a, b, t) => a + (b - a) * (1 - Math.cos(t * Math.PI)) / 2;

var noise = x => {
    x = x * 0.01 % 254;
    return lerb(pts[Math.floor(x)], pts[Math.ceil(x)], x - Math.floor(x));
}

//init
var bgcolor = "#748da6";
var forecolor = "#967E76";
var linecolor = "#9CB4CC";
var linewidth = 3;
var offset = -10;
var yRatio = .9;
var t = 0;
...
var player = new function () {
    ...
    //interface
    this.baslaBtn= new Image();
    this.baslaBtn.src="basla.png";
    ...
    this.draw = function () {
        ...
        if (!playing || gnd && Math.abs(this.rot) > Math.PI * .5) {
            ...
            ctx.drawImage(this.baslaBtn, (c.width / 2)-25, (c.height / 3)+40, 50, 50);
        }
      // rotation calc
        ...
        ctx.rotate(this.rot);
        ctx.drawImage(this.truck, -40, -40, 100, 100);
        ctx.restore();
    }
}
...
function handleStart(evt){
    ...
    for(let i=0; i<touches.length;i++){
        ...
        if(touch.pageX>((c.width/2)-25)&& touch.pageX<((c.width/2)+25)&& touch.pageY>((c.height/3)+40)&& touch.pageY<((c.height/3)+80)){
            console.log("restart");
